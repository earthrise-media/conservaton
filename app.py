...
def load_data(plot=True):
	...
	return properties, vi, defor, water, evapo, fires, pop, soil, carbon


properties, vi, defor, water, evapo, fires, pop, soil, carbon = load_data()
...
rule = alt.Chart(carbon_df).mark_rule(color='red').encode(
    x='mean(avgtc):N'
)
